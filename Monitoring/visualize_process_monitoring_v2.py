...
def visualize_system_monitoring():
    ...
    disk_write, process_name = load_binary_file(input_dir, 'disk_write_', np.int64)
    disk_read, process_name = load_binary_file(input_dir, 'disk_read_', np.int64)
    ...
    # ax0.xaxis.set_major_formatter(DateFormatter("%d. %b %H:%M"))
    ax0.xaxis.set_major_formatter(DateFormatter("%d.%m %H:%M"))
    ax0.set_xlabel('Time [day.month hour:minute]')
    ...
    ax1.set_xlabel('Time [day.month hour:minute]')
    ax1.set_ylabel('RSS memory [GB]')
