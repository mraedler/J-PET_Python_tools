...
def main():
    ...
    (n_angles, radius,
     n_rings, delta_rings,
     n_submodule_lat, delta_submodule_lat,
     n_crystal_lat, delta_crystal_lat,
     n_crystal_zed, delta_crystal_zed,
     depth, transaxial, axial) = get_ge_discovery_mi_parameters(fltnblut, intnblut)

    scanner_name = 'GE_Discovery_MI'
    description = '...'
...
def get_ge_discovery_mi_parameters(fltnblut, intnblut):
    # Rsector
    n_angles = intnblut(34)
    radius = fltnblut(368.)  # [mm]

    # Module
    n_rings = intnblut(4)  # modules
    delta_rings = fltnblut(50.64)  # [mm]

    # Submodule
    n_submodule_lat = intnblut(4)
    delta_submodule_lat = fltnblut(16.125)  # [mm]

    # Crystal
    n_crystal_lat = intnblut(4)
    delta_crystal_lat = fltnblut(3.975)  # [mm]
    n_crystal_zed = intnblut(9)
    delta_crystal_zed = fltnblut(5.3155)  # [mm]

    # Size
    depth = fltnblut(25.)  # [mm]
    transaxial = fltnblut(3.95)  # [mm]
    axial = fltnblut(5.3)  # [mm]
...
def generate_lut(fltnblut, intnblut,
                 ...
                 scanner_name, description):
    #
    n_elements = n_angles * n_rings * n_submodule_lat * n_crystal_lat * n_crystal_zed

    # Ring (module) centers
    ring_centers_zed = symmetric_grid(n_rings, dtype=fltnblut) * delta_rings
    submodule_centers_lat = symmetric_grid(n_submodule_lat, dtype=fltnblut) * delta_submodule_lat
    ...
    #
    z_r, y_sm, z_c, y_c = np.meshgrid(ring_centers_zed, submodule_centers_lat,
                                      crystal_centers_zed, crystal_centers_lat, indexing='ij')

    # #
    # fig, ax = plt.subplots()
    # ax.scatter(z_r + z_c, y_sm + y_c, alpha=0.1)
    # # ax.scatter(z_r, y_sm, alpha=0.1)
    ...
    phi = np.arange(n_angles, dtype=fltnblut) / n_angles * 2 * np.pi
